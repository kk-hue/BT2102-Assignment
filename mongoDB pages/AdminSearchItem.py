...
class AdminSearchItem:
    ...
    def __init__(self, root):
        ...
        self.frame.place(x=390, y=150, height=150, width=450)
        ...
        self.itemIDLabel.place(x=190, y=45)

        self.entry1 = Entry(self.frame, font=("times new roman", 15, 'bold'))
        self.entry1.place(x=100, y=70, width=250)
        ...
        self.searchButton.place(x=150, y=110, width=150)

        # ====== Table Frame ========

        table_frame = Frame(self.frame1, bd=0, relief=RIDGE, bg="crimson")
        table_frame.place(x=10, y=200, width=460, height=100)

        scroll_x = Scrollbar(table_frame, orient=HORIZONTAL)
        scroll_y = Scrollbar(table_frame, orient=VERTICAL)
        self.item_table = ttk.Treeview(table_frame,
                                       columns=("ItemID", "Model",
                                                "Category", "Color",
                                                "PowerSupply", "Factory",
                                                "ProductionYear", "PurchaseStatus", "ServiceStatus"),
    ...
    def admin_search_item(self):
        if (self.entry1.get() == ""):
            ...
            try:
                cursor = collection.find({"ItemID": int(self.entry1.get())})
                if cursor.count() != 0:
                    for index, data in enumerate(cursor):
                        self.item_table.insert('', END, values=(data['ItemID'], data['Model'],
                                                                data['Category'],data['Color'],
                                                                data['PowerSupply'], data['Factory'],
                                                                data['ProductionYear'], data['PurchaseStatus'],
                                                                data['ServiceStatus']))
